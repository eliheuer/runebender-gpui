...
impl Workspace {
    fn font(&self) -> Option<&Master> {
        self.project.as_ref().map(|p| p.active_font())
    }

    fn font_mut(&mut self) -> Option<&mut Master> {
    ...
    fn glyph_codepoints(font: &Master, name: &str) -> Vec<u32> {
        font.font
            .get_glyph(name)
...
",
    ...
    fn check_features_compile(font: &Master, fea: &str) -> Result<(), String> {
