...
always what the document says. A new panel goes in `view/panels/`,
one file per region. The canvas paints in layers, back to front: a
new layer is a new `paint_*` function called from `paint_scene`, not
